...
function longestWords(csvData) {
  ...
  var place = csvData[longest.i][longest.key].text;
  ...
  _createClass(Main, [{
    ...
    value: function setForcer() {
      ...
      if (!thereWasAtLeastOne) {
        ...
        }).font(layoutData.styling["normal"]).font({
          size: 45,
          anchor: 'middle'
        }).move(480, 120).filter(shadow);
      }
    }
...
var SpotlightStage = function () {
  ...
  function SpotlightStage() {
    ...
    this.numSpotlights = imageData.reduce(function (sum, image) {
      return sum + image.spotlights.length;
    }, 0);
  ...
  _createClass(SpotlightStage, [{
    ...
    value: function move(AOarray) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        var spotsNeeded = AOarray.reduce(function (spots, AO) {
        ...
        _this8.turnOffLight(_this8.spotlights.children().filter(function (node) {
          return node.opacity();
        }).length - spotsNeeded.length);

        //move all the spots to their correct spot
        spotsNeeded.forEach(function (spot, i) {
          //var targ = AO.imageData.spotlights[0]
          _this8.spotlights.children()[i].animate(ANIMATION_DURATION).opacity(1).plot(spot).after(function () {
    ...
    value: function createLight() {
      ...
      tempSpot.fill('black').opacity(0).filter(this.filter);
...
var TextController = function () {
  ...
  _createClass(TextController, [{
    ...
    value: function displayText(AOarray) {
      ...
      return new Promise(function (resolve, reject) {
        // display the text
        console.log(AOarray[0].TPdata[_this9.currentTP]);
        $("#message").text(AOarray[0].TPdata[_this9.currentTP].text);
        document.getElementById("details").innerHTML = AOarray[0].imageData.label + " <span>" + (AOarray[0].TPdata[AOarray[0].currentState].value + 1) + " to " + (AOarray[0].TPdata[_this9.currentTP].value + 1) + "</span>";
        resolve(AOarray);
      });
    }
  }, {
...
function drawPoint() {
  var point = list[1][1];
  draw.circle(10).center(point[0], point[1]).fill('orange').stroke({
    color: '#f06',
    width: 4
  });
}
