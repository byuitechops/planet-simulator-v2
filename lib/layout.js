...
function fixSVGSize() {
  ...
  //sort by size;
  var sizes = [svgSizeFromH, svgSizeFromW]
...
var draw = SVG('drawing').size("100%", "100%").viewbox(size.vbX, size.vbY, size.vbWidth, size.vbHeight);
...
layoutData.background.forEach(function (img) {
  background.add(draw.image(img.ref, img.width, img.height).x(img.x).y(img.y));
});

var forcers = draw.group().attr('name', 'forcers');
layoutData.forcers.forEach(function (seg, i) {
  var group = draw.group();
  seg.forEach(function (elm) {
    if (elm.ref) group.add(draw.image(elm.ref, elm.width, elm.height));else group.add(draw.text(elm.text).x(elm.x).y(elm.y).font(layoutData.styling['forcer']));
  });
...
imageData.forEach(function (elm) {
  var i;
  ...
  if (elm.name == 'underwaterVolcano') {
    maskUnderwaterVolcano = draw.mask().attr('name', 'maskUnderwaterVolcano').add(draw.rect(elm.width, elm.height).x(elm.x).y(elm.y).scale(elm.scale.x, elm.scale.y).fill(gradientUnderwaterVolcano));

    elm.handle.maskWith(maskUnderwaterVolcano);
  } else if (elm.name == 'insolation') {
    maskInsolation = draw.mask().attr('name', 'maskInsolation').add(draw.rect(elm.width, elm.height).x(0).y(elm.y).scale(elm.scale.x, elm.scale.y).fill(gradientInsolation));
  ...
  for (i = 1; i <= elm.items; i++) {
    var frame = draw.image(elm.path + i + elm.ext, elm.width, elm.height).opacity(0).x(elm.x).y(elm.y).scale(elm.scale.x, elm.scale.y).data('frame', i - 1).data('macaroni', 'false');
  ...
  if (elm.macaroni.needed) {
    for (i = 1; i <= elm.items; i++) {
      var macaroni = draw.image(mac.path + i + mac.ext, mac.width, mac.height).x(elm.macaroni.x).y(elm.macaroni.y).data('frame', i - 1).opacity(0).data('macaroni', 'true');

      if (elm.macaroni.mirrored) macaroni.scale(-1, 1);
...
layoutData.text.forEach(function (line) {
  text.add(line.font.reduce(function (sum, font) {
    return sum.font(layoutData.styling[font]);
  }, draw.text(line.text).x(line.x).y(line.y).font(layoutData.styling['normal'])));
});
//CO2 meeter needs special attention 
text.add(draw.text(function (add) {
...
text.dy(-7).children().forEach(function (child) {
  return child.filter(shadow);
});
...
layoutData.timeline.forEach(function (seg, i) {
  var time = draw.group();
  seg.forEach(function (elm) {
    if (elm.ref) time.add(draw.image(elm.ref, elm.width, elm.height));else time.add(draw.text(elm.text).x(elm.x).y(12.5).font(layoutData.styling['normal']).font(layoutData.styling['smallText']));
  });
...
if (window.location.href.includes('&home=homepage')) {
  var backToHome = draw.text('Back to home').move(50, 57).fill('#FFFFFF').size(20, 50).font('color', 'red');
