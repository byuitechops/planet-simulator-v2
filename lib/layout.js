...
var shadow = function shadow(add) {
...
imageData.forEach(function (elm) {
	...
	for (var i = 1; i <= elm.items; i++) {
		var frame = draw.image(elm.path + i + elm.ext, elm.width, elm.height).x(elm.x).y(elm.y).scale(elm.scale.x, elm.scale.y).data('frame', i - 1).opacity(0).data('macaroni', 'false');
